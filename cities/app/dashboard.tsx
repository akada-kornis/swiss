...
export default function Dashboard() {
  ...
  const softwares = useMemo(() => [...new Set(municipalities.map((item) => item.software).filter(Boolean))].sort(), [municipalities]);
  const filtered = useMemo(() => municipalities.filter((item) => {
    ...
    return (!needle || `${item.name} ${item.canton} ${item.software} ${item.integrator}`.toLocaleLowerCase("fr-CH").includes(needle))
      && (canton === "Tous" || item.canton === canton) && (software === "Tous" || item.software === software)
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <section className="kpi-grid" aria-label="Indicateurs clés">
        ...
        <article className="kpi-card prime-card"><div className="kpi-top"><span className="kpi-label">Clients Prime</span><span className="prime-mark">P</span></div><strong>{stats.prime}</strong><p>{number.format(stats.primePopulation)} habitants identifiés</p></article>
        <button className="kpi-card alert-card" onClick={() => setIssuesOnly(true)}><span className="kpi-label">Livraisons à surveiller</span><strong>{stats.issues}</strong><p>statut différent de « accepté » <span>Voir →</span></p></button>
      </section>
      <section className="data-panel">
        ...
        <div className="filters">
          ...
          <label><span>Logiciel</span><select value={software} onChange={(event) => setSoftware(event.target.value)}><option>Tous</option>{softwares.map((value) => <option key={value}>{value}</option>)}</select></label>
          <button className={`filter-toggle ${primeOnly ? "on" : ""}`} onClick={() => setPrimeOnly(!primeOnly)}>Clients Prime</button><button className={`filter-toggle ${issuesOnly ? "on warning" : ""}`} onClick={() => setIssuesOnly(!issuesOnly)}>À surveiller</button>
        </div>
        ...
        <div className="table-wrap"><table><thead><tr><th>État</th><th>Commune</th><th>Canton</th><th>Population</th><th>Logiciel</th><th>Intégrateur</th><th>Erreur EWID</th><th /></tr></thead><tbody>{filtered.slice(0, 120).map((item) => <tr key={item.id} onClick={() => setSelected(item)}><td><StatusDot status={item.deliveryStatus} /></td><td><strong>{item.name}</strong>{item.isPrime && <span className="client-pill">Prime</span>}</td><td>{item.canton}</td><td className="numeric">{number.format(item.expectedPopulation)}</td><td>{item.software || <span className="empty">À compléter</span>}</td><td>{item.integrator || <span className="empty">—</span>}</td><td><span className={`rate ${(item.ewidErrorRate ?? 0) > 1 ? "high" : ""}`}>{item.ewidErrorRate?.toFixed(1) ?? "—"}%</span></td><td className="arrow">›</td></tr>)}</tbody></table>{filtered.length > 120 && <p className="table-limit">120 premiers résultats affichés · affinez les filtres pour aller plus loin</p>}</div>
      </section>
    </section>
    {selected && <div className="drawer-backdrop" onClick={() => setSelected(null)}><aside className="drawer" onClick={(event) => event.stopPropagation()}><button className="drawer-close" onClick={() => setSelected(null)}>×</button><div className="drawer-title"><StatusDot status={selected.deliveryStatus} /><div><p>{selected.canton} · OFS {selected.id}</p><h2>{selected.name}</h2></div></div><div className="drawer-pop"><strong>{number.format(selected.expectedPopulation)}</strong><span>habitants attendus</span></div><section><h3>Livraison Delimo</h3><dl><div><dt>Population reçue</dt><dd>{number.format(selected.receivedPopulation ?? 0)}</dd></div><div><dt>Erreur EWID</dt><dd>{selected.ewidErrorRate?.toFixed(1)}%</dd></div><div><dt>EWID manquants</dt><dd>{selected.missingEwid?.toFixed(1)}%</dd></div><div><dt>Version eCH</dt><dd>{selected.echVersion}</dd></div></dl><p className="delivery-comment">{selected.comment}</p></section><section><h3>Connaissance marché</h3><label>Logiciel installé<input defaultValue={selected.software} placeholder="À compléter" /></label><label>Intégrateur<input defaultValue={selected.integrator} placeholder="À compléter" /></label><label>Statut commercial<select defaultValue={selected.salesStatus}><option value="none">Non qualifié</option><option>À cibler</option><option>En discussion</option><option>Client</option></select></label><label>Notes<textarea defaultValue={selected.notes} placeholder="Informations utiles, contexte, prochaine étape…" /></label><button className="save-button">Enregistrer les informations</button><p className="prototype-note">V0 locale · l’enregistrement sera activé avec la base de données</p></section></aside></div>}
  </main>;
}
