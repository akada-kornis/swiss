  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      <section className="kpi-grid" aria-label="Indicateurs clés">
        ...
        <button className={`kpi-card alert-card ${ofsMode ? "ofs-active" : ""}`} onClick={() => { setOfsMode(!ofsMode); if (ofsMode) setIssuesOnly(false); }}><span className="kpi-label">Contrôle OFS</span><strong>{ofsMode ? stats.issues : "Ouvrir"}</strong><p>{ofsMode ? "Revenir à la vue commerciale" : "Afficher les statuts Delimo"} <span>{ofsMode ? "←" : "→"}</span></p></button>
      </section>
